...
import { createServer } from "http";
 ...
 // Quiz de alemão - 40 perguntas organizadas por nível
const questions = [
 //  A1 Level Questions (0-9)
  ...
  { id: 28, question: "Das Projekt _____ bis morgen fertig sein.", options: ["muss", "hat", "braucht"], answer: "A", level: "B1" },
  ...
  { id: 37, question: "gestern / wegen der Arbeit / habe ich / zu Hause / viel gearbeitet.", options: [
    "ich habe gestern zu Hause wegen der Arbeit viel gearbeitet",
    "ich habe zu Hause gestern wegen der Arbeit viel gearbeitet",
    "ich habe wegen der Arbeit gestern zu Hause viel gearbeitet"
  ], answer: "A", level: "B2" },
...
export default async function handler(req, res) {
  // Extraímos dados do corpo da requisição
  const { phase, questionIndex, answer, score } = req.body;
  ...
  if (phase === "feedback") {
    ...
    const dicas = {
      A1: "• Reforce artigos (der/die/das, ein) e gêneros\n• Präsens de sein/haben e ordem S-V-O\n• Ja-Nein-Fragen e W-Fragen",
      A2: "• Pratique Perfekt vs Präteritum\n• Casos acusativo vs dativo e preposições\n• Verbos modais e Imperativo",
      B1: "• Use Konjunktiv II para hipóteses e pedidos polidos\n• Orações subordinadas com weil, obwohl, als ob\n• Pronomes relativos e Präteritum comum",
      B2: "• Aplique voz passiva e Modalpassiv\n• Partizipialkonstruktionen e genitivo\n• Conjunções correlativas e inversões estilísticas"
    };

    const feedback = `Você acertou ${score} de 40 e seu nível estimado é **${nivel}**. Parabéns!\n\n` +
      `**Próximas etapas para ${nivel}:**\n${dicas[nivel]}\n\n` +
      `Quer ir além? Entre no meu WhatsApp e garanta condição especial para o Curso Completo de Alemão da Ovídio Academy:\n` +
...
// Se executar diretamente, inicia servidor Express
if (require.main === module) {
  const app = express();
  app.use(express.json());
  app.post("/api/quiz", handler);
  app.use(express.static("public"));
  app.get("*", (req, res) => res.sendFile("public/index.html", { root: process.cwd() }));
  const port = process.env.PORT || 3000;
  createServer(app).listen(port, () => console.log(`> Server rodando em http://localhost:${port}`));
}